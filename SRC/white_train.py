...
#    target = r'C:/Users/Student/Downloads/UVA/Fourth Year/Project/Project 3/PHOTOS/UTKFace/DATA/All races - Female/TRAIN/'+items
...
def train_data_white():
    train_data_white_female = []
    train_data_white_male = []
    for image1 in tqdm(os.listdir(train_white_female)):
        path = os.path.join(train_white_female, image1)
        img1 = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
        img1 = cv2.resize(img1, (image_size, image_size))
        train_data_white_female.append(img1)
    for image2 in tqdm(os.listdir(train_white_male)):
        path = os.path.join(train_white_male, image2)
        img2 = cv2.imread(path, cv2.IMREAD_GRAYSCALE)
        img2 = cv2.resize(img2, (image_size, image_size))
        train_data_white_male.append(img2)

    train_data_white = np.concatenate((np.asarray(train_data_white_female), np.asarray(train_data_white_male)), axis=0)
